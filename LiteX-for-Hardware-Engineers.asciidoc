...
* For "Ports", if listed as a simple name, then it's specifiable as a function parameter.
...
* i: input (from an asynchronous clock domain)
* o: output
* odmain (default: sys): output clock domain
* n (default 2): depth of flip flop chain
* reset (default 0): reset state
...
* idomain: input clock domain (typicaly a string, like "sys" for cd_sys)
* odomain: output clock domain (also a string)
* i _implicit_: input signal to synchronize
* o _implicit_: output signal to synchronize
