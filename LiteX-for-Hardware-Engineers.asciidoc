...
You can add CSRs to modules (but not the top level SoC instantiation).

CSRs are a bit odd, by default they are byte-wide registers that are on 32-bit word
boundaries. So a "32-bit" CSR is actually broken into four bytes spanning a total
address space of 16 bytes.

To be written -- example on how to create CSRs, access bits, get them
into your clock domain, and prevent false timing paths
