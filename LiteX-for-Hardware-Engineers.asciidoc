...
class MySoC(BaseSoC):
    ...
    def __init__(self, ...):
        ...
        self.submodules.analyzer = LiteScopeAnalyzer(analyzer_signals,
                                                     analyzer_depth,
                                                     cd=analyzer_clock_domain)

    # 3. Add this function to your SoC definition to generate the analyzer definition file.
    # If `do_exit()` exists, it will get called by the builder automatically.
...
translates e.g. UART, PCI express, or Wthernet to wishbone addresses and
vice versa.
